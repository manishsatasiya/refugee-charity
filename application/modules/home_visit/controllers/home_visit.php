    	...
    	if($data){
    		foreach($data->result_array() AS $result_row){
    			...
    			if($this->session->userdata('role_id') == '1' || in_array("edit",$this->arrAction)) {
    				$action_btn .= '<li><a href="'.base_url('home_visit/add/'.$result_row["id"]).'" class=""><i class="fa fa-edit"></i> '.$this->lang->line('edit').'</a></li>';
    			}
    			$action_btn .= '<li><a href="javascript:;" onclick=dt_delete("home_visit","id",'.$result_row["id"].'); class=""><i class="fa fa-trash-o"></i> '.$this->lang->line('delete').'</a></li>';
    			$action_btn .= '</ul></div>';
				$row[] = $result_row['id'];
